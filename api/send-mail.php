...
if (!salon_mail_configured()) {
    salon_json_out(['ok' => false, 'error' => 'Gmail nog niet gekoppeld — open setup-mail.php'], 503);
}
...
if ($body === '') {
    salon_json_out(['ok' => false, 'error' => 'Bericht ontbreekt'], 400);
}

$result = salon_smtp_send($to, $subject, $body, $bcc !== '' ? $bcc : null);
if (!$result['ok']) {
